  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!data.success || !data.amount) {
        setStatus("error");
        setMessage(
          "Payment capture failed. Contact support with PayPal order ID: " + token
        );
        return;
      }
      ...
      if (error) {
        ...
        setStatus("error");
        setMessage(
          "Payment captured but order save failed. Contact support with: " + token
        );
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-chaos-darker flex items-center justify-center">
      <div className="text-center space-y-4 p-8">
        <h1 className={`text-3xl font-black ${styles[status]}`}>{message}</h1>
        {status === "success" && (
